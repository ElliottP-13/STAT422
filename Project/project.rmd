...
top_ten_counties <- counties_matrix[,1][order(counties_matrix[,2])][47:56]
top_ten_counties <- rev(top_ten_counties)

##All counties
counties_list <- c()
for(i in 1:dim(counties_matrix)[1]){
  counties_list <- append(counties_list, 
                          rep(i,
                              times = counties_matrix[i,2])) #counties_matrix[i,1] 
}
##Top 10 counties
...
for(i in 1:length(top_ten_counties)){
  top_ten_counties_list <- append(top_ten_counties_list,
                                  rep(i,
                                      counties_matrix[,2][which(counties_matrix[,1]==top_ten_counties[i])]))
}
```
...
```{r}
dist <- function(P, Q){  # KL-Divergence
...
run_bootstraps <- function(reps, counties){
  ...
  for(i in 1:reps){
    for(n in n_list){
      ...
      dists[counter] = dist(bootstrap_pop, counties)
      counter = counter + 1

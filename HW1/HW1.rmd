...
$$F_X(x) = \int_{-\infty}^x f_X(y) dy = \int_{\Theta}^x e^{-y-\Theta} dy = \left( -e^{-y-\Theta} \right]_{\Theta}^{x} = -e^{-x-\Theta} + e^{-2\Theta}$$
...
\begin{align*}
  F_{X_{(1)}}(x) &= P(X_{(1)} \leq x)
   ...
   &= 1 - (1 - F_{X_i}(x))^n \\
   &= 1 - (1 + e^{-x-\Theta} - e^{-2\Theta})^n \\
\end{align*}

2. 
$$f_{X_{(1)}}(x) = \frac{d}{dx}F_{X_{(1)}}(x) = \frac{d}{dx} (1 + e^{-x-\Theta} - e^{-2\Theta})^n ) = n (1 + e^{-x-\Theta} - e^{-2\Theta})^{n-1}e^{-x-\Theta} I_{(\Theta, \infty)}(x)$$
